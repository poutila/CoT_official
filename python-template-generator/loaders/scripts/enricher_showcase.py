...
def analyze_markdown_file(file_path: Path) -> dict[str, Any]:
    ...
    analysis = {
        ...
        "context": {
            "with_context": len(
                [e for e in doc.full_examples if e.context_before or e.context_after]
            ),
            ...
            analysis["context"]["samples"].append(
                {
                    ...
                    "before": example.context_before[:50] + "..."
                    if example.context_before
                    else "None",
                    "after": example.context_after[:50] + "..."
    ...
    analysis["code_extraction"]["sections_with_code"] = list(
        analysis["code_extraction"]["sections_with_code"]
    )
    analysis["example_detection"]["patterns_found"] = list(
        analysis["example_detection"]["patterns_found"]
    )
