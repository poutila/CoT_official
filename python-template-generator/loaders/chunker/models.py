...
class ChunkMetadata(BaseModel):
    ...
    section: Optional[str] = Field(None, description="Section slug where chunk appears")
    subsection: Optional[str] = Field(None, description="Subsection if applicable")
    
    # Code-related metadata
    has_code: bool = Field(False, description="Whether chunk contains code")
    code_languages: List[str] = Field(default_factory=list, description="Programming languages in chunk")
    example_types: List[str] = Field(default_factory=list, description="Types of examples (good/bad/neutral)")
    
    # Position information
    start_line: Optional[int] = Field(None, description="Starting line in source")
    end_line: Optional[int] = Field(None, description="Ending line in source")
...
class Chunk(BaseModel):
    ...
    overlap_prev: Optional[str] = Field(None, description="Overlapping text from previous chunk")
    overlap_next: Optional[str] = Field(None, description="Overlapping text from next chunk")
    overlap_token_count: int = Field(0, description="Tokens in overlap sections")
    
    # Relationships
    prev_chunk_id: Optional[str] = Field(None, description="ID of previous chunk")
    next_chunk_id: Optional[str] = Field(None, description="ID of next chunk")
    parent_chunk_id: Optional[str] = Field(None, description="ID of parent chunk if hierarchical")
