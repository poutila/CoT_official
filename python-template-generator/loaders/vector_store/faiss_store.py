...
class FAISSVectorStore(BaseVectorStore):
    ...
    def __init__(self, config: VectorStoreConfig | None = None, index_type: str | None = None):
        ...
        if config is None:
            config = VectorStoreConfig(
                store_type=VectorStoreType.FAISS, index_type=index_type or "Flat"
            )
        elif index_type:
    ...
    def _initialize_store(self) -> None:
        ...
        try:
            ...
            raise ImportError("faiss-cpu is not installed. Install it with: uv add faiss-cpu")
    ...
    def search(
        ...
        for i in range(len(indices[0])):
            ...
            result = SearchResult(vector_id=vector_id, score=score, metadata=meta, vector=vector)
