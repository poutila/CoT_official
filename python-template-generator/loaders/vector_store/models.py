...
class VectorStoreConfig(BaseModel):
    ...
    store_type: VectorStoreType = Field(
        default=VectorStoreType.FAISS, description="Type of vector store"
    )
    dimension: int = Field(default=384, description="Dimension of vectors")
    distance_metric: DistanceMetric = Field(
        default=DistanceMetric.L2, description="Distance metric for similarity"
    )
    index_type: str | None = Field(
        default=None, description="Specific index type (e.g., 'Flat', 'IVF', 'HNSW')"
    )
    persist_directory: Path | None = Field(
        default=None, description="Directory for persisting the index"
    )
    batch_size: int = Field(default=100, description="Batch size for bulk operations")
    normalize_embeddings: bool = Field(
        default=False, description="Whether to normalize embeddings before storing"
    )
    additional_params: dict[str, Any] = Field(
        default_factory=dict, description="Additional store-specific parameters"
    )
...
class SearchResult(BaseModel):
    ...
    vector_id: str = Field(description="ID of the matched vector")
    score: float = Field(description="Similarity score (distance or similarity)")
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Metadata associated with the vector"
    )
    vector: list[float] | None = Field(default=None, description="The actual vector (if requested)")
...
class SearchBatch(BaseModel):
    ...
    query_vector: list[float] | None = Field(
        default=None, description="Query vector used for search"
    )
...
class IndexMetadata(BaseModel):
    ...
    total_vectors: int = Field(default=0, description="Total number of vectors in the index")
    dimension: int = Field(description="Dimension of vectors")
    index_type: str = Field(description="Type of index being used")
    distance_metric: DistanceMetric = Field(description="Distance metric being used")
    created_at: datetime = Field(
        default_factory=datetime.now, description="When the index was created"
    )
    last_updated: datetime = Field(
        default_factory=datetime.now, description="When the index was last updated"
    )
    storage_size_bytes: int | None = Field(default=None, description="Size of the index in bytes")
    is_trained: bool = Field(
        default=False, description="Whether the index has been trained (for some index types)"
    )
    additional_info: dict[str, Any] = Field(
        default_factory=dict, description="Additional index-specific information"
    )
...
class StorageMetrics(BaseModel):
    ...
    total_adds: int = Field(default=0, description="Total number of vectors added")
    total_searches: int = Field(default=0, description="Total number of searches performed")
    total_deletes: int = Field(default=0, description="Total number of vectors deleted")
    avg_add_time_ms: float = Field(default=0.0, description="Average time to add a vector")
    avg_search_time_ms: float = Field(default=0.0, description="Average search time")
    cache_hits: int = Field(default=0, description="Number of cache hits (if caching enabled)")
...
class VectorBatch(BaseModel):
    ...
    ids: list[str] = Field(description="List of vector IDs")
    metadata: list[dict[str, Any]] = Field(
        default_factory=list, description="List of metadata for each vector"
    )
