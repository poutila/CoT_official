...
class FullCodeExample(CodeExample):
    ...
    original_block_id: Optional[str] = Field(None, description="ID of original block if split")
...
class FullEnhancedDoc(ExampleEnhancedDoc):
    ...
    full_examples: List[FullCodeExample] = Field(
        ...
        description="Complete examples with no limitations"
    )
    ...
    def examples_with_context(self) -> int:
        ...
        return len([ex for ex in self.full_examples 
                   if ex.context_before or ex.context_after])
...
class FullEnhancedEnricher(ExampleEnhancedEnricher):
    ...
    def _split_multi_example_block(self, block: CodeBlock, block_id: int) -> List[FullCodeExample]:
            ...
            if good_match:
                ...
                if current_lines and current_type:
                    ...
                    examples.append(self._create_example(
                        current_lines, block, current_type, current_markers,
                        is_split=True, block_id=str(block_id)
                    ))
                current_lines = [line]
        ...
        if not examples:
            ...
            example_type, markers = self._comprehensive_pattern_detection(
                block.content, "", ""
            )
            
            examples.append(FullCodeExample(
        ...
        for i, node in enumerate(self.context_map):
            if node['type'] == 'fence' and block.content in node['content']:
                ...
                if i < len(self.context_map) - 1:
                    for j in range(i+1, min(len(self.context_map), i+5)):  # Look ahead up to 5 nodes
    ...
    def get_detailed_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pattern_summary": {
                ...
                "neutral": len([e for e in doc.full_examples if e.example_type == ExampleType.NEUTRAL]),
                "comparison": len([e for e in doc.full_examples if e.example_type == ExampleType.COMPARISON])
            },
            "languages": list(set(e.language for e in doc.full_examples if e.language)),
            "sections_with_examples": list(set(e.section_slug for e in doc.full_examples if e.section_slug))
        }
...
def test_full_enricher():
    ...
    avg_time = sum(r['elapsed_time'] for r in all_results.values()) / len(all_results) if all_results else 0
    ...
    success = (
        total_examples >= 10 and  # Found substantial examples
        (total_good + total_bad) > 0 and  # Found classified examples
        avg_time < 2.0  # Performance target met
    )
