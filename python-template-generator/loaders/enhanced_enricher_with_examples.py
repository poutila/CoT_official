...
import re
from minimal_enhanced_enricher import (
...
class ExampleEnhancedDoc(MinimalEnhancedDoc):
    ...
    code_examples: List[CodeExample] = Field(
        ...
        description="Code blocks with example classification"
    )
    ...
    def pattern_summary(self) -> Dict[str, int]:
        ...
        return {
            ...
            "neutral": len([e for e in self.code_examples if e.example_type == ExampleType.NEUTRAL]),
...
def test_example_detection():
    ...
    try:
        ...
        print(f"  - Languages with examples:")
        for lang, counts in stats["languages_with_examples"].items():
            print(f"    {lang}: {counts['total']} total ({counts['good']} good, {counts['bad']} bad)")
