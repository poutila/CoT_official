...
class RAGConfig(BaseModel):
    ...
    # Document loading
    loader_type: str = Field(default="markdown", description="Type of document loader")

    # Chunking parameters (maps to LangChain's TextSplitter)
    chunk_size: int = Field(default=512, description="Maximum size of chunks (in tokens)")
    chunk_overlap: int = Field(default=50, description="Overlap between chunks (in tokens)")
    separators: list[str] | None = Field(
        default=None, description="Separators for splitting (like LangChain's separators)"
    )
    ...
    index_type: str = Field(default="Flat", description="Index type for vector store")
    persist_directory: Path | None = Field(
        default=None, description="Directory to persist the vector store"
    )

    # Retrieval parameters (maps to LangChain's retriever)
    k: int = Field(default=4, description="Number of documents to retrieve")
    search_type: str = Field(
        default="similarity", description="Type of search (similarity, mmr, threshold)"
    )
    score_threshold: float | None = Field(
        default=None, description="Minimum similarity score threshold"
    )
    fetch_k: int = Field(
        default=20, description="Number of documents to fetch before filtering (for MMR)"
    )
    lambda_mult: float = Field(
        default=0.5, description="Lambda for MMR (0=max diversity, 1=max relevance)"
    )

    # Pipeline parameters
    verbose: bool = Field(default=False, description="Whether to print verbose output")
    return_source_documents: bool = Field(
        default=True, description="Whether to return source documents with results"
    )
    max_tokens_limit: int | None = Field(
        default=2000, description="Maximum tokens for context window"
    )
...
class RetrievalResult(BaseModel):
    ...
    scores: list[float] | None = Field(
        default=None, description="Similarity scores for each document"
    )
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata about the retrieval"
    )
...
class IndexingResult(BaseModel):
    ...
    time_elapsed: float = Field(description="Time taken in seconds")
...
class RAGResponse(BaseModel):
    ...
    answer: str | None = Field(default=None, description="Generated answer (if using LLM)")
...
class PromptTemplate(BaseModel):
    ...
    input_variables: list[str] = Field(description="List of input variable names")
