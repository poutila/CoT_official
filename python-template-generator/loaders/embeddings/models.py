...
class EmbeddingProviderConfig(BaseModel):
    ...
    provider_type: EmbeddingProviderType = Field(
        default=EmbeddingProviderType.SENTENCE_TRANSFORMER, description="Type of embedding provider"
    )
    model_name: str = Field(default="all-MiniLM-L6-v2", description="Model name or path")
    dimension: int | None = Field(
        default=None, description="Embedding dimension (auto-detected if not provided)"
    )
    ...
    device: str | None = Field(default=None, description="Device to use (cuda/cpu/auto)")
    cache_embeddings: bool = Field(default=True, description="Whether to cache embeddings")
    max_seq_length: int | None = Field(default=None, description="Maximum sequence length")
...
class EmbeddingMetadata(BaseModel):
    ...
    token_count: int | None = Field(default=None, description="Number of tokens in input")
    truncated: bool = Field(default=False, description="Whether input was truncated")
    processing_time_ms: float | None = Field(
        default=None, description="Time taken to generate embedding"
    )
...
class EmbeddingBatch(BaseModel):
    ...
    model_name: str = Field(description="Model used for all embeddings")
    total_processing_time_ms: float = Field(description="Total time for batch processing")
    ...
    def search(
        self, query: EmbeddingResult | list[float] | np.ndarray, top_k: int = 5
...
class EmbeddingCache(BaseModel):
    ...
    cache: dict[str, EmbeddingResult] = Field(default_factory=dict, description="Cache storage")
