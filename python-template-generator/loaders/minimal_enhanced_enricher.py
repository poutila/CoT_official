    ...
    def _extract_code_blocks(self) -> List[CodeBlock]:
        ...
        for node in self.tree.walk(include_self=False):
            ...
            if node.type == 'heading' and node.children:
                heading_text = ''.join(
                    child.content for child in node.children 
    ...
    try:
        ...
        if doc.code_blocks:
            ...
            for i, cb in enumerate(doc.code_blocks[:3], 1):
                ...
                print(f"    Preview: {cb.content[:50]}..." if len(cb.content) > 50 else f"    Content: {cb.content}")
                ...
                "code_blocks": [
                    {
                        ...
                        "content_preview": cb.content[:100] + "..." if len(cb.content) > 100 else cb.content
                    }
                    for cb in doc.code_blocks
                ]
            }
