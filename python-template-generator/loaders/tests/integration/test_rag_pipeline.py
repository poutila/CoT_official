...
def test_retrieval_methods(rag_pipeline: RAGPipeline):
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        # Retrieve documents
        result = rag_pipeline.retrieve(query, k=2)

        print(f"  Found {len(result.documents)} relevant documents:")
        for j, (doc, score) in enumerate(zip(result.documents, result.scores, strict=False), 1):
...
def test_langchain_compatible_interface(rag_pipeline: RAGPipeline):
    """Test LangChain-compatible interfaces."""
    print_section("3. LANGCHAIN COMPATIBILITY TEST")

    # Test invoke method
    print("Testing invoke() method:")
    response = rag_pipeline.invoke({"query": "What are the benefits of RAG systems?"})
...
def test_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config = RAGConfig(chunk_size=100, persist_directory=save_path)
...
def test_performance_metrics():
    ...
    for query in queries:
        ...
        print(
            f"  - Query: '{query[:30]}...' -> {len(result.documents)} docs in {retrieval_time * 1000:.1f}ms"
        )
