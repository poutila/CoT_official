...
class RAGPipeline:
    ...
    def load_documents(self, paths: Union[Path, List[Path]]) -> List[Document]:
                ...
                chunk_doc = Document(
                    ...
                    metadata={
                        **doc.metadata,  # Preserve original metadata
                        ...
                        "chunk_index": chunk.metadata.chunk_index if hasattr(chunk.metadata, 'chunk_index') else 0,
                        "token_count": chunk.token_count,
                        "chunk_type": chunk.chunk_type.value if hasattr(chunk, 'chunk_type') else "text"
                    }
                )
    ...
    async def ainvoke(self, inputs: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        return await asyncio.get_event_loop().run_in_executor(
            None, self.invoke, inputs, kwargs
        )
